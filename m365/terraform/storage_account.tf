locals {
  sa_prefix    = replace(local.prefix, "-", "")
  sa_unique_id = substr(replace(local.app_client_id, "-", ""), 0, 24 - length(local.sa_prefix))

  network_rules = local.ip_rules == null ? null : object({
    bypass                     = ["AzureServices"]
    default_action             = "Deny"
    ip_rules                   = local.ip_rules
    virtual_network_subnet_ids = var.vnet == null ? null : [module.networking[0].aci_subnet_id]
  })
}
